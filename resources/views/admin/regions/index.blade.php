{{-- resources/views/admin/regions/index.blade.php --}}
...
@media (max-width: 480px) {
    ...
    .region-table tbody td:last-child {
        border-bottom: none;
    }
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="region-header">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold mb-2">Gestion des Régions</h1>
                <p class="opacity-90 text-lg">Administrez les régions et leurs caractéristiques</p>
            </div>
            <a href="{{ route('admin.regions.create') }}" class="btn-add">
                <span>🗺️</span> Ajouter une région
            </a>
        </div>
    </div>
    ...
    <div class="stats-container">
        <div class="stat-card">
            <div class="stat-value">{{ $regions->count() }}</div>
            <div class="stat-label">Total des régions</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ $regions->sum('population') ?? 0 }}</div>
            <div class="stat-label">Population totale</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ $regions->sum('superficie') ?? 0 }} km²</div>
            <div class="stat-label">Superficie totale</div>
        </div>
        <div class="stat-card">
            <div class="stat-value">{{ $regions->filter(fn($region) => !empty($region->localisation))->count() }}</div>
            <div class="stat-label">Avec localisation</div>
        </div>
    </div>
                ...
                <tr id="region-{{ $region->id }}">
                    ...
                    <td data-label="Localisation">
                        @if($region->localisation)
                            <span class="localisation-badge" title="{{ $region->localisation }}">
                                {{ $region->localisation }}
                            </span>
                        @else
                            <span class="text-gray-400">-</span>
                ...
                <tr>
                    <td colspan="6">
                        <div class="empty-state">
                            ...
                            <h3 class="text-2xl font-bold text-gray-800 mb-3">Aucune région enregistrée</h3>
                            <p class="empty-state-text">Commencez par ajouter votre première région</p>
                            <a href="{{ route('admin.regions.create') }}" class="btn-add mt-4">
                                <span>➕</span> Ajouter la première région
                            </a>
                        </div>
                    </td>
                </tr>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tableRows = document.querySelectorAll('.region-table tbody tr');
...
document.addEventListener('submit', function(e) {
    if (e.target.closest('form[action*="regions"]') && e.target.method === 'post') {
        const form = e.target;
            ...
            if (confirm('Êtes-vous sûr de vouloir supprimer cette région ?')) {
                ...
                fetch(form.action, {
                    ...
                    if (response.ok) {
                        ...
                        showNotification('Région supprimée avec succès', 'success');
                    } else {
