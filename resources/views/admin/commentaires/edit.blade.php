...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg w-full p-8 transform transition hover:scale-105 hover:shadow-2xl">
        ...
        <h1 class="text-2xl font-bold mb-6 text-gray-800 text-center">Modifier un commentaire</h1>
        ...
        <form action="{{ route('admin.commentaires.update', $commentaire->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" name="note" min="0" max="5" value="{{ $commentaire->note }}"
                       class="w-full bg-gray-50 border border-gray-300 rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <select name="id_utilisateur"
                        ...
                        required>
                    @foreach($users as $u)
                        <option value="{{ $u->id }}" {{ $u->id == $commentaire->id_utilisateur ? 'selected' : '' }}>
                            {{ $u->nom }} {{ $u->prenom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="id_contenu"
                    ...
                    @foreach($contenus as $c)
                        <option value="{{ $c->id }}" {{ $c->id == $commentaire->id_contenu ? 'selected' : '' }}>
                            {{ $c->titre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex gap-3 justify-end mt-4 flex-wrap">
                <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg shadow-md transition flex items-center justify-center">
                    Mettre à jour
                </button>
